...
public class InstantSend {
    ...
    */
    ...
    /*public void processTransactionLockRequest(Peer pfrom, TransactionLockRequest tx) {
        if (!canProcessInstantSendMessages())
            return;
    ...
    */
            ...
            if(!mapTxLockCandidates.containsKey(txHash)) {
                ...
                if(!mapMasternodeOrphanVotes.containsKey(vote.getOutpointMasternode())) {
                    ...
                    if(nPrevOrphanVote > Utils.currentTimeSeconds() && nPrevOrphanVote > getAverageMasternodeOrphanVoteTime()) {
                        log.info("instantsend--CInstantSend::ProcessTxLockVote -- masternode is spamming orphan Transaction Lock Votes: txid="+txHash);//+"  masternode=", vote.getOutpointMasternode().toStringCpp());
                ...
                for(Sha256Hash hash : it1)
                {
                    if(hash != txHash) {
                        ...
                        if(it2.hasMasternodeVoted(vote.getOutpoint(), vote.getOutpointMasternode())) {
                            ...
                            // pose ban score too.
                            log.info("CInstantSend::ProcessTxLockVote -- masternode sent conflicting votes! ");//+ vote.getOutpointMasternode().toStringCpp());
                            return false;
                        }
                    }
                }
                ...
                if (it != null) {
                    for(Sha256Hash hash : it)
                    {
                        if (!hash.equals(txLockRequest.getHash())) {
                            log.info("instantsend", "CInstantSend::ProcessTxLockRequest -- Double spend attempt! %s"+ txin.getOutpoint().toStringCpp());
    ...
    boolean isLockedInstantSendTransaction(Sha256Hash txHash)
    {
        if(!context.allowInstantXinLiteMode() /*|| fLargeWorkForkFound || fLargeWorkInvalidChainFound */||
                !context.sporkManager.isSporkActive(SporkManager.SPORK_2_INSTANTX)) return false;
        ...
        try {
            ...
            while (itVote.hasNext())
            {
                ...
                if (vote.getValue().isExpired(currentBlock.getHeight())) {
                    log.info("instantsend--CInstantSend::CheckAndRemove -- Removing expired vote: txid="+vote.getValue().getTxHash()+"  masternode=" + vote.getValue().getOutpointMasternode().toStringCpp());
            ...
            while (itOrphanVote.hasNext())
            {
                ...
                if (Utils.currentTimeSeconds() - vote.getValue().getTimeCreated() > ORPHAN_VOTE_SECONDS) {
                    log.info("instantsend--CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid="+vote.getValue().getTxHash()+"  masternode="+ vote.getValue().getOutpointMasternode().toStringCpp());
